        ...
        IClinicalTrialMetadataService clinicalTrialMetadataService) : ControllerBase
    {
        ...
        public async Task<IActionResult> Get([FromQuery] string[] trialId, [FromQuery] ClinicalTrialStatus[] status)
        {
            ...
            return await pagination.MatchAsync(
                ...
                () => Task.FromResult(
                    ServiceResult<ClinicalTrialMetadata>
                        .FromError(
                            Application.Factories.ProblemDetailsFactory.CreateBadRequest("Pagination is not valid. Specify it using headers: x-pagination-offset (any whole number) and x-pagination-limit (whole number from 0 to 50)"))
                        .ToResponse()
               ));
